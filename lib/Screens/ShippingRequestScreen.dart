...
class ShippingRequestScreen extends StatefulWidget {
  ...
  /// are something from outside. Null shows the chooser as before.
  final ShippingSource? initialSource;
...
class _ShippingRequestScreenState extends State<ShippingRequestScreen> {
  ...
  ShippingStep _step = ShippingStep.values.first;

  /// How far they have been, so the header lets them tap back but not skip.
  ShippingStep _furthest = ShippingStep.values.first;
  ...
  void _back() {
    if (_step == ShippingStep.values.first) {
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      canPop: _step == ShippingStep.values.first,
    ...
    return Row(
      children: [
        for (final step in ShippingStep.values) ...[
          if (step != ShippingStep.values.first)
            Expanded(
              child: Container(
                height: 2,
